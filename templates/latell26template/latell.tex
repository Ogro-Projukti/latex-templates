...
\title{Instructions for LaTeLL 2026 Proceedings}
...
\maketitle
\begin{abstract}
This document contains the instructions for preparing a manuscript for the proceedings of LaTeLL 2026.
...
The following instructions are directed to authors of papers submitted to LaTeLL 2026 or accepted for publication in its proceedings.
...
LaTeLL provides this description and accompanying style files at
\begin{quote}
\url{https://latell.org/2026/latell2026/latell2026-templates.zip}
\end{quote}
We strongly recommend the use of these style files, which have been appropriately tailored for the LaTeLL 2026 proceedings. You can also start from the Overleaf template linked from the conference website. 

\paragraph{\LaTeX-specific details:}
The templates include the \LaTeX2e{} source (\texttt{\small latell.tex}),
the \LaTeX2e{} style file used to format it (\texttt{\small latell.sty}),
an ACL bibliography style (\texttt{\small acl\_natbib.bst}),
an example bibliography (\texttt{\small latell.bib}),
...
LaTeLL publishes Regular papers 8 pages (with 30 min oral presentation), and Short papers 4 pages (with 20 min oral presentation).

\textbf{Additional pages are allowed for references only. Upon acceptance, final versions of all submission types will be given one additional content page to address the reviewers' comments. Appendices are not recommended unless they are very important for the quality of the paper.}

\textbf{Papers that do not conform to the specified length and formatting requirements are subject to rejection without review.}
...
\section{Anonymity}
As reviewing will be double-blind (except for demos), papers submitted for review should not include any author information (such as names or affiliations). Furthermore, self-references that reveal the author's identity, \emph{e.g.},
\begin{quote}
...
If your own bib file is named \texttt{\small latell.bib}, then placing the following before any appendices in your \LaTeX{}  file will generate the references section for you:
\begin{quote}\small
\verb|\bibliographystyle{acl_natbib}|\\
\verb|\bibliography{latell}|
\end{quote}
...
\begin{quote}\small
\verb|\bibliographystyle{acl_natbib}|\\
\verb|\bibliography{anthology,latell}|
\end{quote}
...
\textbf{Cross-referencing}: To create a cross-reference for an equation:

\begin{itemize}
    \item Create a bookmark for it.
    \item Select the number to the right of the equation. Go to \textbf{Insert, Bookmark} (in the \textbf{Links} panel), and then create a name for your equation. Press \textbf{Add} to create the bookmark.
    \item To refer back, place the mouse pointer at the location where you wish to add the cross reference.
    \item Go to \textbf{Insert, Cross-reference} (in the \textbf{Links} panel). In the dialogue box, select \textbf{Bookmark} and \textbf{Bookmark Text} from each dropdown list. Uncheck \textbf{Insert as Hyperlink}, then click \textbf{OK}.
    \item This will make it such that whenever a new equation is added, the references to the equation will be updated when \texttt{Ctrl-A + F9} is pressed.
    \item This is an example cross-reference to Equation~\ref{eq:example}.
\end{itemize}
...
(1) {\small\verb|\RequirePackage{etoolbox}|} (line 455 in \texttt{\small latell.sty}), and
(2) A long line below (line 456 in \texttt{\small latell.sty}).
...
\noindent
{\small\verb|\usepackage[nohyperref]{acl2025}|}
