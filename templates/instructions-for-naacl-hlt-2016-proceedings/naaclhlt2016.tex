...
the {\small\verb|\naaclfinalcopy|} command in the document preamble.)  Reviewers:
note that the ruler measurements do not align well with lines in the paper
--- this turns out to be very difficult to do well when the paper contains
many figures and equations, and, when done, looks ugly.  Just use fractional
references (e.g the first line on this page is $096.5$), although in most cases one would
...
in this release ({\small\tt naalhlt2016.bib}).  Some example commands
for names with accents are provided in Table \ref{tab:accents}.  

Provide as complete a reference as possible, with a consistent format,
...
References should appear at the 
end of the document, but before any Appendices (unless the appendices contain references).
References stored in the separate~{\small\tt .bib} file, for example, {\small\tt naalhlt2016.bib}, are inserted into the document using the following commands:
...
References should appear under
the heading {\bf References} and should be arranged alphabetically
...
sequentially in the form:  ``{\bf Figure 1:} Caption of the Figure.'' ``{\bf Table 1:} Caption of the Table.''  Type the captions of the figures and 
...
The NAACL HLT 2016 main conference accepts submissions of long papers
