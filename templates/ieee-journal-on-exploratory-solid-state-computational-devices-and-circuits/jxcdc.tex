...
\makeatletter
\def\ps@headings{\def\@oddhead{\vbox{\vspace{17pt}\hsize\textwidth\hbox{\rfxfont\rightmark\hfill}\hfill\par
\smallskip\noindent\hbox to \textwidth{\vrule width\textwidth height.3pt depth0pt}}}%
\def\@evenhead{\vbox{\vspace{17pt}\hsize\textwidth\hfill\hbox{\hfill\rhfont\leftmark}\par
\smallskip\noindent\hbox to \textwidth{\vrule width\textwidth height.3pt depth0pt}}}%
\def\@oddfoot{\hfill\rffont\thepage}\def\@evenfoot{\rffont\thepage\hfill}}
\def\ps@plain{\def\@oddhead{\vbox{\vspace{17pt}\hsize\textwidth\hbox{\rhfont\leftmark\hfill}\hfill\par
\smallskip\noindent\hbox to \textwidth{\vrule width\textwidth height.3pt depth0pt}}}%
\def\@evenhead{\vbox{\vspace{17pt}\hsize\textwidth\hfill\hbox{\hfill\rhfont\leftmark}\par
\smallskip\noindent\hbox to \textwidth{\vrule width\textwidth height.3pt depth0pt}}}%
\def\@oddfoot{\hfill\rffont\thepage}\def\@evenfoot{\rffont\thepage\hfill}}
\makeatother
\AtBeginDocument{\definecolor{ojcolor}{cmyk}{1,0.45,0,0.18}%
\definecolor{ojcolor2}{cmyk}{0,0.91,0.81,0.19}%
\pagestyle{headings}\thispagestyle{plain}}
\markboth{IEEE Journal on 
\textcolor{ojcolor2}{Exploratory Solid-State}
\textcolor{ojcolor}{Computational Devices and Circuits}}
{Author {et al.}: Preparation of Papers for IEEE Journal on Exploratory Solid-State Computational Devices and Circuits}
\begin{document}
...
\doiinfo{JXCDC.2023.3238030}

\title{\textcolor{ojcolor2}{Preparation of Papers for IEEE Journal
...
{http://ieeeauthorcenter.ieee.org/create-your-ieee-article/}\break
\underline{use-authoring-tools-and-ieee-article-templates/ieee-article-}\break\underline{templates/}
so you can use it to prepare your manuscript.  If you would prefer to
use Microsoft \textit{Word} (versions 6.0 or later), download template
and sample files from the same Web page.  You can also explore using
the Overleaf editor at
...
Try not to use too many typefaces in the same formula.
Also please remember that MathJax
can't handle nonstandard typefaces or symbols.
...
Be careful with labels. If you assign the same label to a
subsubsection and a table, you might find that Table I has been cross
referenced as Table IV-B3.

Also be careful with the location of labels. If you put a
...
The SI unit for magnetic field strength \(H\) is A/m. However, if you wish to use units of T, either refer to magnetic flux density \(B\) or magnetic field strength symbolized as \({\mu }_{0 }H\). Use the center dot to separate compound units, e.g., ``A\({\cdot }\)m\(^{2 }\).''
...
Be aware of the different meanings of the homophones ``affect'' (usually a verb) and ``effect'' (usually a noun), ``complement'' and ``compliment,'' ``discreet'' and ``discrete,''
``principal'' (e.g., ``principal investigator'') and ``principle'' (e.g., ``principle of measurement''). Do not confuse ``imply'' and ``infer.''

Prefixes such as ``non,'' ``sub,'' ``micro,'' ``multi,'' and ``ultra'' are not independent words; they should be joined to the words they modify, usually without a hyphen. There is no period after the ``et'' in the Latin abbreviation ``\textit{et al.}'' (it is also italicized). The abbreviation ``i.e.,'' means ``that is,'' and the abbreviation ``e.g.,'' means ``for example'' (these abbreviations are not italicized).
...
coincide with those listed above. \textit{Proceedings of the IEEE} has a column 
...
(only U.S. State), Country: Abbrev. of Publisher, year, ch. \textit{x}, sec. \textit{x}, pp.
\textit{xxx--xxx.}
...
J. K. Author, ``Name of paper,'' \textit{Abbrev. Title of
Periodical}, vol. \textit{x}, no. \textit{x}, pp. \textit{xxx-xxx}, Abbrev. Month, year, DOI.
...
\begin{thebibliography}{00}
\bibitem{b12} \textit{Name of Manual/Handbook}, \textit{x} ed., Abbrev. Name of Co., City of Co., Abbrev. State, Country, year, pp.
\textit{xxx-xxx.}
\end{thebibliography}
...
Publisher, State, Country: Abbrev. of Publisher, year, ch.
\textit{x}, sec. \textit{x}, pp.
\textit{xxx--xxx}. [Online]. Available: \underline
{http://www.web.com}. Accessed on: Month Day, Year.
...
Founders' Constitution. }Chicago, IL, USA: Univ. of Chicago Press, 1987, Accessed on: Feb. 28, 2010. [Online] Available: {http://press-pubs.uchicago.edu/founders/}
\end{thebibliography}

\subsubsection*{Basic format for journals (when available online):}

J. K. Author, ``Name of paper,'' {\textit{Abbrev. Title of Periodical}}, vol. \textit{x}, no. \textit{x}, pp. {\textit{xxx-xxx}}, Abbrev. Month, year. Accessed on: Month, Day, year, DOI: 10.1109.{\textit{XXX}}.123456, [Online].

\subsubsection*{Examples:}

\begin{thebibliography}{00}
\bibitem{b19b} J. S. Turner, ``New directions in communications,'' \textit{IEEE J. Sel. Areas Commun}., vol. 13, no. 1, pp. 11-23, Jan. 1995. 
\bibitem{b19c} W. P. Risk, G. S. Kino, and H. J. Shaw, ``Fiber-optic frequency shifter using a surface acoustic wave incident at an oblique angle,'' \textit{Opt. Lett.}, vol. 11, no. 2, pp. 115--117, Feb. 1986.
\bibitem{b19d} P. Kopyt \textit{et al., ``}Electric properties of graphene-based conductive layers from DC up to terahertz range,'' \textit{IEEE THz Sci. Technol.,} to be published. DOI: 10.1109/TTHZ.2016.2544142.
\end{thebibliography}
...
\begin{enumerate}
\item J. K. Author, ``Title of thesis,'' M.S. thesis, Abbrev. Dept., Abbrev.
Univ., City of Univ., Abbrev. State, year. p. nnn.
\item J. K. Author, ``Title of dissertation,'' Ph.D. dissertation, Abbrev.
...
\begin{thebibliography}{00}
\bibitem{b50} S. Azodolmolky, \textit{et al.}, Experimental demonstration of an impairment aware network planning and operation tool for transparent/translucent optical networks,''~\textit{J. Lightwave. Technol.}, vol. 29, no. 4, pp. 439--448, Sep. 2011.
\end{thebibliography}
